...
def report_manage(request):
    ...
    Returns:
        html -- html response
    """
    form_name = request.GET.get('t', None)
    keyword = request.POST.get('keyword', None)
    if form_name not in ['trouble_report', 'daily_report']:
    ...
    if keyword:
        if form_name == 'trouble_report':
            res = res.filter(
                Q(info__contains=keyword) |
                Q(reason__contains=keyword) |
                Q(resolv_method__contains=keyword)
            )
        if form_name == 'daily_report':
            res = res.filter(
                content__contains=keyword
            )

    if export:
        wb = export_to_file(form_name, res)
        response = HttpResponse(content_type='application/vnd.ms-excel')
        response['Content-Disposition'] = 'attachment;filename=%sReport.xls' % form_name
        output = BytesIO()
        wb.save(output)
    ...
    return render(request, 'admin/list_%s.html' % form_name, {
        'users': user_data,
